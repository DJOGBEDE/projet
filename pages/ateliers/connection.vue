<template>
  <v-container class="fill-height d-flex align-center justify-center" fluid>
    <v-card class="pa-5" min-width="800">
      <v-card-title class="headline text-center">Connexion Atelier</v-card-title>
      <v-card-text>
...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      try {
        const response = await axios.post('http://localhost:8080/api/login/atelier', {
          email: this.email,
        ...
        this.$router.push('/ateliers/dashbord');
      } catch (error) {
